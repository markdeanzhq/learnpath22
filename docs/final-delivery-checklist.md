...
> 对应 `align-profile-resource-workflow` 后续优化，适合答辩前手动跑通一遍并截图留证。

1. 创建“我想系统学习机器学习基础”项目，确认目标候选后进入画像采集。
2. 在画像中选择标准路径、混合资料和中等练习密度，生成最新学习路径。
3. 在 Path 页确认路径模式来源、阶段解释和推荐资源；若存在空阶段，截图保留 `empty_reason`。
4. 在 Path 页搜索资料并绑定到当前知识点，确认绑定后回到推荐资源页签展示。
5. 在 Dashboard 展开对应知识点的“学习资料 N 条”，记录开始或完成状态。
6. 切换到 Search 页，确认其定位为项目资料库/搜索历史，并将一条资料转为图谱草稿来源。
7. 在 Knowledge 页打开 overlay 抽屉，生成或复用候选，确认预检面板展示校验失败、需复核、待审核和未开启规划的状态说明。
8. 点击“修复校验失败”或“复核需确认”，确认只打开候选筛选/编辑入口，不直接改变审核或 planning 状态。
9. 如有已确认且开启规划的候选，从 Knowledge 进入 Path 基础/增强图谱路径对比。
10. 触发一次进度感知重规划，确认已完成节点被保留，剩余路径可解释调整。
