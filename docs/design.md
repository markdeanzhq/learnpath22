...
- `in_domain_uncovered` 返回 `draft_proposal` 推荐草稿预览与 `draft_entry.presentation=draft_inbox`；项目创建为 `extension_review` 后进入 Knowledge 草稿收件箱，用户显式确认后才物化为 project overlay session
- `draft_proposal` 只包含可审核的 nodes / edges / resources 候选、gap analysis、review notes 与安全 metadata，不创建 overlay source/session，不写正式图谱或正式路径；当前规则型补边只生成保守 `RELATED_TO` 候选，不自动生成高风险 `REQUIRES` 硬依赖
- 已有项目可在 Knowledge 手动触发当前目标覆盖分析；只有 project-scoped preview 返回 `review_extension_draft` 时才打开推荐草稿收件箱，仍需用户再次点击创建才物化 overlay session
- `in_domain_uncovered` 物化后的草稿复用 project overlay lifecycle；只有通过 validation、人工 confirmed 且 `planning_enabled=true` 后才可能进入增强图谱规划
...
问卷答案会被确定性映射为画像参数并落库。规划权威画像维度包括：`math_level`、`coding_level`、`ml_level`、`theory_weight`、`practice_weight`、`weekly_hours`、`deadline_weeks`。

`path_mode_preference`、`learning_goal_orientation`、`resource_preference`、`practice_intensity`、`persona_label`、`persona_summary`、`persona_evidence` 属于展示、解释与 audit/report 快照字段；其中 `practice_intensity` 用于表达练习强度证据，当前不改写 planner numeric scoring。
...
增强图谱预检器在生成基础/增强图谱方案对比前聚合 active overlay、planner-visible overlay、实际进入增强快照的 overlay、最新路径命中的 overlay，以及被 baseline 覆盖、端点不可用或关系重复而忽略的边。若增强快照包含 `REQUIRES` 环依赖，preflight 返回 `blocked`，前端禁用图谱方案对比；若存在待审核、未开启规划、校验失败、基线覆盖或 inactive 关系，则返回 `warning` 并提示用户先处理。
...
- overlay pasted text / search URL source 持久化，并保留手动补充资料入口
...
推荐草稿、LLM 抽取预览与规则补边都属于 project overlay 的候选输入层。它们不能直接进入 `ProjectGraphSnapshot`，也不能直接影响路径生成；只有被物化为候选、通过校验、人工确认并开启 planning 的 overlay node/edge，才会出现在增强图谱方案和后续 planner 输入中。
...
- 资源推荐读取最新学习者画像，`resource_preference` 会影响搜索 query hint、`preference_match` 标记和轻量分数提升；非偏好但内容相关的资料不会被过滤
- `/search` 明确定位为项目资料库与搜索历史，负责保存和复用资料；`/path` 负责把资料绑定到阶段/知识点；`/knowledge` 负责将资料转成 overlay 草稿来源并进入人工审核
...
- Dashboard 进度页按知识点展开已绑定资源，资源加载失败不会阻断开始、完成或跳过等进度操作
...
- `GET /api/v1/health/readiness` 返回分层 readiness 语义，区分本地主链、Neo4j 投影和在线增强能力
- 前端 `normalizeReadiness` 对旧联调实例保留兼容归一化逻辑，设置页按三块能力卡片展示可演示范围
...
- `local_demo_ready`：SQLite 与本地 Domain Pack 读模型是否足以支撑项目创建、图谱浏览与路径规划主链演示
- `core_ready` / `demo_ready`：保留旧语义，表示 SQLite、Neo4j 与 `graph_sync` 是否全部就绪
- `enhanced_ready`：LLM 与搜索等在线增强能力是否就绪
- `ready`：只有当旧 `demo_ready && enhanced_ready` 同时满足时才为 `true`，用于完整环境就绪判断
- `capabilities.local_graph_read`：本地主链能力卡片的数据源
- `capabilities.neo4j_projection`：Neo4j 投影同步、诊断与推广能力卡片的数据源
- `capabilities.online_enhancement`：LLM 与搜索在线增强能力卡片的数据源

其中 `services.graph_sync` 直接来自图谱同步状态查询，用于说明 Neo4j 中的 Domain Pack 主图与扩展实体是否与当前代码包保持同步。答辩演示时应优先观察 `local_demo_ready` 与 `capabilities.local_graph_read`；Neo4j 投影或在线增强未就绪时，不代表本地图谱浏览和路径规划主链不可用。
