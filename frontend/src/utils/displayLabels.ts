...
const SESSION_STATUS: Record<string, DisplayMeta> = {
  ...
  validated: { label: '校验完成', tagType: 'warning', detail: '候选已完成机器校验，但仍需处理失败项并人工确认' },
...
const ERROR_CODE: Record<string, string> = {
  ...
  INVALID_LLM_EXTRACTION_JSON: 'AI 抽取结果格式无效，请重新生成预览或改用手动草稿',
  LLM_NOT_READY: 'LLM 尚未配置，无法生成扩展抽取预览',
  LLM_EXTRACTION_FAILED: 'LLM 抽取暂时失败，请稍后重试或缩短资料内容',
  ...
  OVERLAY_CANDIDATE_NOT_FOUND: '扩展候选不存在',
  OVERLAY_CANDIDATE_PATCH_EMPTY: '请至少修改一个候选字段',
  OVERLAY_CANDIDATE_PROMOTED_READ_ONLY: '已推广候选不可编辑',
  ...
  projection_missing: '已有扩展草稿，但尚未同步为 Neo4j 投影；普通浏览和路径预检不受影响，需要显式同步时再点击同步图谱',
  overlay_projection_drifted: '扩展草稿已变化，Neo4j 投影等待重新同步',
  no_overlay: '暂无项目扩展草稿',
  synced: '已同步',
  seed_metadata_stale: '图谱同步信息已过期',
  source_context_sparse: '资料内容较少，AI 抽取结果可能为空或需要人工补充',
  neo4j_unavailable: 'Neo4j 暂不可用；本地图谱浏览不受影响，投影同步相关能力会受限',
